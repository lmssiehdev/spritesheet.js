...
var algorithms = {
  ...
  'multiple-binpacking': multipleBinpacking,
...
function multipleBinpacking(files, options) {
  var atlas = null,
    atlases = [],
    index = 0,
    packer = null;
  ...
  while (files.length) {
    packer = new binpacking.GrowingPacker(options.width, options.height);
    packer.fit(files);
    atlas = {
      files: []
    };
    files.forEach(function (item) {
      if (item.fit) {
        ...
        delete item.w;
        delete item.h;
        atlas.files.push(item);
      }
    });
    atlas.width = packer.root.w;
    atlas.height = packer.root.h;
    atlases.push(atlas);
    files = packer.nofit;
    index += 1;
  }
