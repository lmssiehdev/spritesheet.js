...
exports.determineCanvasSize = function (files, options, callback) {
	...
	if (options.atlases) {
		options.atlases.forEach(function (atlas) {
			if (options.square) {
				atlas.width = atlas.height = Math.max(atlas.width, atlas.height);
			}

			if (options.powerOfTwo) {
				atlas.width = roundToPowerOfTwo(atlas.width);
				atlas.height = roundToPowerOfTwo(atlas.height);
			}
		});
	} else {
		if (options.square) {
...
exports.generateMultiples = function (exporter, options, callback) {
	var arr = [];
	...
	options.atlases.forEach(function (item, index) {
		arr.push(function (callback) {
			options.width = item.width;
			options.height = item.height;
			options.name = options.baseName + '-' + index;
			exporter(item.files, options, callback);
		});
	});
